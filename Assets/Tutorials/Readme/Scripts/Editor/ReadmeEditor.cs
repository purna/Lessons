        ...
        public static void DrawInspectorGUI(Readme readme)
        {
            ...
            foreach (var section in readme.sections)
            {
                ...
                if (!string.IsNullOrEmpty(section.ImageText))
                {
                        ...
                        int oldHeight = section.ImageIcon.height;
                        int oldWidth = section.ImageIcon.width;

                    float aspect = (float)oldHeight / (float)oldWidth;
                    int newHeight = (int)(Screen.width * aspect);

                    //(New Width / Old Width) *Old Height

                    GUILayout.Box(section.ImageIcon, GUILayout.Height(newHeight/2), GUILayout.MaxWidth(Screen.width), GUILayout.ExpandWidth(true));
